...
void solve(){
	...
	//Let's place the elements with max frequency at first (say a and b, there could be more though)
	// ab_______ab_______ab_______ab________ab
	//Now the remaining elements should be placed in the blank spaces one by one
	vll v(n, 0);
	map<ll, ll> f1, ff1;
	rep(i, n){
		cin>>v[i];
	...
	ll freq = 0;
	trav(elem, f1) (freq = (elem.s>=freq)?elem.s:freq);

	cout << ff1[freq]-1+(n-ff1[freq]*freq)/(freq-1) << '\n';
	return;
}
