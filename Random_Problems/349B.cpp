...
void solve(){
	...
	//Find the digit which can give max freq: In case of conflict: Choose with low cost
	//Start replacing with greater nos, from paint left and cost of paint at ith pos.
	ll digMax;
	ll freqMax = 0;
	rep(i, n){
		if (freqMax<=p/cost[i])
		{
			if (freqMax==p/cost[i])
			{
				if (!freqMax || cost[i]>cost[digMax-1])
				{
					continue;
				}
			}
