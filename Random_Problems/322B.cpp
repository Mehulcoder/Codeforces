...
void solve(){
	...
	vector<ll> segSum(n, 0);
	vector<pll> maxFromHere(n, {0,0});
	...
	ll currMax = -INF;
	repr(i, n-k){
		if (segSum[i]>=currMax || i==n-k)
		{
			currMax = segSum[i];
			maxFromHere[i] = {segSum[i], i};
		}else{
			maxFromHere[i] = maxFromHere[i+1];
		}
	}

	trace(segSum, maxFromHere);

	ll ans = -INF;
	ll a,b;
	a=b=-1;
	rep(i, n-2*k+1){
		ll curr = segSum[i]+maxFromHere[i+k].f;
		if (ans<curr)
		{
			ans = curr;
			a = i+1;
			b = maxFromHere[i+k].s+1;
		}
	}
