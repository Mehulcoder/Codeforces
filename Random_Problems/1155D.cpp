...
void solve(){
	...
	//Iterate on right and left while keeping the best left possible for that right
	// ---------|-------|---------------
	//If we are choosig a segment we'll obviously include it in our answer completely
	...
	fr(i, 0, n-1){
		if (i==0)
		{
			bestLeft = max(bestLeft, 0ll);
		}else{
			bestLeft = max(bestLeft, l[i-1]-x*s[i-1]);
		}
		if (i==n-1)
		{
			ans = max(ans, x*s[i]+bestLeft);
		}else{
			ans = max(ans, r[i+1]+x*s[i]+bestLeft);

		}

		//It might be the case that we need not to mult by x at all
		ans = max(ans, l[i]);
	}
