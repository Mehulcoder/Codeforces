...
vector<vector<ll>> heKnows;
vector<vector<ll>> languageBois;
...
void create(){
	...
	rep(i, n){
		trav(lang, heKnows[i]){
			trav(boi, languageBois[lang]){
				if (boi!=i)
				{
					edges[i].push_back(boi);
				}
			}
		}
	}
...
void solve(){
	...
	rep(i, n){
		...
		rep(j, k){
			...
			ll l;
			cin>>l;
			languageBois[l-1].push_back(i);
			heKnows[i].push_back(l-1);
		}
	}
	if (!flag)
	{
