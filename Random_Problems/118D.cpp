...
ll dp[101][101][11][11];
ll get(ll nh, ll nf, ll kh, ll kf){
	if(nh+nf==0) return 1;
	ll &ans = dp[nh][nf][kh][kf];
	if(ans!=-1) return ans;

	ans = 0;
	if(nh-1>=0 && kh-1>=0) ans+=get(nh-1, nf, kh-1, k2)%MOD;
	if(nf-1>=0 && kf-1>=0) ans+=get(nh, nf-1, k1, kf-1)%MOD;
...
void solve(){
	cin>>n1>>n2>>k1>>k2;
	fil(dp, -1);
	cout << get(n1, n2, k1, k2)%((ll)MOD) <<endl;
	return;
}
