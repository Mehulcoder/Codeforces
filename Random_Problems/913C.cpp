...
void solve(){
	ll n, l, fAns = INF; cin>>n>>l;
	
	vector<pair<ld, ll>> v(n, {0.00, 0});
	rep(i,n){
		cin>>v[i].f;
		if(i>0) v[i].f = min(2*v[i-1].f, v[i].f);
	}
	rep(i, n){
		ll num = poww(2, i);
		v[i].f/=(num*1.0);
	...
	ll rem = l, cost = 0, i=0;
	while(i<n && rem>0){
		cost+=(v[i].f)*((rem/v[i].s)*v[i].s);
		rem-=(rem/v[i].s)*v[i].s;
		//Now if I add the count of curr again the num of bottles will exceed
		//This might be the best option, and always possibly an option
		fAns = min(fAns, cost+ll((v[i].f)*v[i].s*(rem>0)));
		i++;
	}
