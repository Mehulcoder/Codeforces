...
void solve(){
	...
	vset(adj, n, vll(n, 0));
	vset(x, n, 0);
	vset(dist, n, vll(n, INT_MAX));
	vset(existin, n, 0);
	ans.clear();
	
	//Use Floyd warshal to calculate minimum distance when k is the intermediate vertex
	//Think of it as what vertex do we have to add in bw from end to start
	rep(i, n){
		rep(j, n){
			cin>>adj[i][j];
			dist[i][j] = adj[i][j];
