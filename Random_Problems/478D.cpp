...
void solve(){
	...
	//Methods for making height i and with total j reds available
	//dp[i][j] = dp[i-1][j] + dp[i-1][j-i]
	vset(dp2, r+1, 0);
	
	//Basically setting dp[0][0] as 1 (number of ways to form tower of h 0 with 0 r)
	//We assume that we'll have enough greens.
	dp2[0] = 1;
	fr(i,1, h){
		frr(j, r, i){
			dp2[j] += dp2[j-i]%MOD;
