...
void solve(){
	...
	trav(elem,  heads){
		...
		headTail.push_back({elem, tail});
	}
	// trace(headTail);

	ll head = headTail[0].f;
	ll finalTail = headTail[0].s;
	ll finalHead = headTail[0].f;
	fr(i, 1, headTail.size()-1){
		v[headTail[i].s] = head;
		// trace(headTail[i], head);
		head = headTail[i].f;
		finalHead = head;
	}
	v[finalTail] = finalHead;
	fr(i, 1, n){
		cout << v[i] << ' ';
	}
	cout <<  '\n';
