...
void solve(){
	...
	//Size will be odd always
	//Last index will be even always
	n = v2.size();
	trace("here");
	rep(i, v2.size()){
		if (i%2==0)
	...
	maxi = max(maxi, -geteve(0, n-1)+getodd(0, n-1)+v2[0]-1);
	trace(maxi, eve, odd, v2);
	rep(i, n-1){
		ll temp = geteve(0, i)-getodd(0, i)+getodd(i+1, n-1)-geteve(i+1, n-1);
		if (i%2!=0)
		{
			temp+=getVal(i);
		}else{
