...
void getSum(ll root, auto &visited){
	visited[root] = 1;

	trav(child, edges[root]){
		if (!visited[child])
		{
			getSum(child, visited);
...
ll check(ll root, auto &visited){
	...
	trav(child, edges[root]){
	 ...
	 if ((sums[root]+h[root])%2!=0)
	 {
	 	return -1;
	 }
	 if (sums[root]-h[root]<0)
	 {
	 	return -1;
	 }
	 if (g<=(sums[root]+h[root])/2)
	 {
	 	return (sums[root]+h[root])/2;
	 }
