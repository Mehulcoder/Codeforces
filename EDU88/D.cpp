...
void solve(){
	...
	fr(i, 0, 30){
		...
		//dp[j] will store the maximum possible sum starting from jth index in v, such that the 
		//maximum is i. Now you just need to check for every index before the last occurance index
		//of i, in v and get the maximum (dp[j]-i)
		repr(j, n-2){
			if (v[j]>i)
			{
				dp[j] = 0;
			}else{
				dp[j] = max(0ll,v[j]+dp[j+1]);
			}
		}
		//Last index of i
		ll ind = -1;
