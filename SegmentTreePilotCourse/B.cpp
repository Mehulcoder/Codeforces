...
void build(ll start, ll tl, ll tr){
	...
	t[start] = min(t[2*start+1],t[2*start+2]);
	return;
}
...
ll get(ll start, ll tl, ll tr, ll l, ll r){
	if (tl>=r || tr<=l)
	{
		return INF;
	}
	...
	ll mid = (tl+tr)/2;
	ll left = get(2*start+1, tl, mid, l, r);
	ll right = get(2*start+2, mid, tr, l, r);
