...
ll getMax(ll id, ll curr, vector<ll> &v, vector<vector<ll>> &dp, ll n, ll x){
    if (id == n) 
        return 0; 
    ...
    if (curr == 0) { 
        ...
        ans = max(ans, v[id] + getMax(id + 1, 0, v, dp, n, x)); 
        ans = max(ans, x * v[id] + getMax(id + 1, 1, v, dp, n, x)); 
    } 
    else if (curr == 1) { 
  
        ans = max(ans, x * v[id] + getMax(id + 1, 1, v, dp, n, x)); 
  
        ans = max(ans, v[id] + getMax(id + 1, 2, v, dp, n, x)); 
    } 
    else
  
        ans = max(ans, v[id] + getMax(id + 1, 2, v, dp, n, x)); 
  
    return dp[id][curr] = ans; 
}
...
void solve(){
	...
	ll ans = 0;
	rep(i, n){
		ans = max((ll)ans, (ll)getMax(i, 0, v, dp, n, x));
	}
	cout << ans << '\n';
	return;
}
