...
ll get(string &s, ll start, ll end, char gc){
	if (gc-'a'>25)
	{
		return s.size();
	}
	...
	ll ans2 = 0;
	//rightHalf
	fr(i, (start+end+1)/2, end){
		ans1+=s[i]!=gc;
	}

	ans1+=get(s, start, (start+end-1)/2, char(gc+1));
	//leftHalf
	fr(i, start, (start+end-1)/2){
		ans2+=s[i]!=gc;
	}

	ans2+=get(s, (start+end+1)/2, end, char(gc+1));
