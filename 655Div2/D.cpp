...
void solve(){
	...
	//Choose (n+1)/2 elements such that
	// Only one pair is adjacent
	//prefix[i]->prefix sum ending at i
	//suffix[i]->suffix sum starting at i
	vector<ll> v(n);
	...
	suffix[n-2] = v[n-2];
	fr(i, 2, n-1){
		prefix[i] = prefix[i-2]+v[i];
	}

	frr(i, n-3, 0){
		suffix[i] = suffix[i+2]+v[i];
	}

	//Now choose the starting position of the adjacent pair as the index of dp
	vector<ll> dp(n, 0);
	rep(i, n){
		if (i-2>=0 && i!=n-1)
		{
	...
	trace(suffix, prefix);
	trace(max_element(all(dp))-dp.begin());
