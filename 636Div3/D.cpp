...
void solve(){
	...
	vector<ll> big;
	rep(i, n/2){
		small.push_back(min(v[i], v[n-i-1]));
		big.push_back(max(v[i], v[n-i-1]));
	}
	sort(all(small));
	sort(all(big));
	ll ans = INF;
	...
	fr(sum, 2, 2*k){
		ll lambda = 0;
		lambda += sms-(lower_bound(all(small), sum)-small.begin());
		lambda += lower_bound(all(big), sum-k)-big.begin();
		ll cost = lambda+(n/2-m1[sum]);
		ans = min(ans, cost);
	}
